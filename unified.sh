...
# Release type
if [ $BRANCH == "nontreble" ]; then
        export TYPE=NonTreble
elif [ $BRANCH == "treble" ]; then
        export TYPE=treble
elif [ $BRANCH == "pie" ]; then
        export TYPE=pie
else
export TYPE=test-NonTreble
...
cp $(pwd)/out/arch/arm64/boot/Image.gz-dtb $(pwd)/anykernel
        cd anykernel
        zip -r9 SolarisKernel-SANTONI-$TYPE-$YOLO.zip * -x README.md SolarisKernel-SANTONI-$TYPE.zip

        curl -F chat_id="-1001151761414"  \
                    -F caption="sha1sum: $(sha1sum SolarisKernel*.zip | awk '{ print $1 }')" \
                    -F document=@"$(pwd)/SolarisKernel-SANTONI-$TYPE-$YOLO.zip" \
                    https://api.telegram.org/bot1105001387:AAEb1sgfaKcP1Hd4-9yDBTNZNxfzFnp05pM/sendDocument

        rm -rf SolarisKernel-SANTONI-$TYPE-$YOLO.zip  && rm -rf Image.gz-dtb
