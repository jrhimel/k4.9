				...
				struct buffer_info *binfo)
{
	...
	for (i = 0; i < binfo->num_planes; i++) {
		...
		if (!skip) {
			rc = msm_smem_cache_operations(inst->mem_client, binfo->handle[i], offset, size, cache_op);
			if (rc) {
				dprintk(VIDC_ERR,
			...
			struct buffer_info *buffer_info)
{
	...
	for (i = 0; i < b->length; i++) {
		...
		if (!skip && size > 0) {
			rc = msm_smem_cache_operations(inst->mem_client, buffer_info->handle[i], offset, size, cache_op);
			if (rc) {
				dprintk(VIDC_ERR,
